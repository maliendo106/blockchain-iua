...
/// @title Game
contract Game {
    enum Status {created, running, ended}
    ...
    event GameTerminated(address direccion);
    ...
    modifier onlyBalanceZero () {
        require(address(this).balance == 0, "winnings not transfered yet");
        _;
    }
    ...
    function join() virtual public payable isBet {
    ...
    function transferWinnings(address payable player) internal onlyEnded {
    ...
    function transferAllWinnings() internal onlyEnded{
        for (uint256 index = 0; index < players.length; index++) {
            transferWinnings(players[index]);
        }
    }
    ...
    function kill() public onlyCreator {
        ...
        uint256 balance = address(this).balance;
        require(balance == 0, "can't kill a game with winnings");
        emit GameTerminated(msg.sender);
        selfdestruct(players[0]); // corregido creo
    }
}
